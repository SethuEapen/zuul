...
bool Parser::processCommand(char* str, Player* player){//this is the command that is used when you enter something in to the console it executes the commands
  if(strncmp(str, "go ", 3) == 0){//I took this idea from andrew chesters code - finds if the first 3 characters are "go "
	...
	if(strcmp(player->getCurrentRoom()->getName(), "Interogation Room") == 0){//make sure player is in same room as curious george
									  ...
									  if((strcmp(str6, str2) != 0) && (strcmp(str6, str3) != 0) && (strcmp(str6, str4) != 0) && (strcmp(str6, str5) != 0)){//same as before
										if(player->validItem(str6)){
										  if(player->keyItem(str6)){
											  ...
											  cout << "Curious George applauds your efforts and is happy to announce that you brought him all the correct items:" << endl;
											  cout << "\"I knew you would understand what items to bring. It was quite obvious actualy the hat was becuase of the man with the yellow hat, " << endl;
											  cout << "The banana was because I eat bananas." << endl;
											  cout << "The pumpkin was a head for no noggin, obviously a reference to the Halloween boofest episode" << endl;
											  cout << "The bath toys were from the episode where I cloged the pluming of the whole building beucase I wanted bath toys season 2 episode 10" << endl;
											  cout << "And the snow machine was an obvious reference to the presant that I tried to make for the man with the yellow hat in A Very Monkey Christmas" << endl;
											  cout << "My henchmen will now escort you out\"" << endl << "YOU WIN!!!!!!";
											  return false;
										  }
										}
									  }
		...
		cout << "\"I always knew you were the worst coder in the class and I am not suprized to see that you failed\"" << endl;
		cout << "Galbraith picks you up by your shirt collar and drags you to the cells. He thows you inside and locks it." << endl;
		cout << "He then chews the key up and spits it on the ground and walks away" << endl << "You lose!!!!!!";
		return false;
	}
	else{
	  cout << "You need to be in the interogation room to give the items to George" << endl;
	  return true;
	}
  }
